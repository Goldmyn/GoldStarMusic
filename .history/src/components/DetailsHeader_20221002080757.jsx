...
const DetailsHeader = ({ artistId, artistData, songData }) => (
  <div className="relative w-full flex flex-col">
    ...
    <div className="absolute inset-0 flex items-center">
      ...
      <div className="ml-5">
        <p className="font-bold sm:text-3xl text-xl text-yellow-400">
          {artistId ? artistData?.artists[artistId].attributes?.name : songData?.title}
        </p>
        {!artistId && (
          <Link to={`/artists/${songData?.artists[0]?.adamid}`}>
            <p className="text-base text-yellow-100 mt-2">{songData?.subtitle}</p>
          </Link>
        )}

        <p className="text-base text-yellow-400 mt-2">
          {artistId
